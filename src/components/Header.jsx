  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled || open ? 'bg-white/95 backdrop-blur-md shadow-md' : 'bg-transparent'
      }`}
    >
      <div className="container-page flex h-20 items-center justify-between">
          ...
          {links.map((l) => (
            <a
              ...
              className={`text-sm font-semibold transition-colors hover:text-brand-orange ${
                scrolled || open ? 'text-brand-blue-deep' : 'text-white'
              }`}
            >
              {l.label}
            </a>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="hidden lg:inline-flex items-center gap-2 rounded-full bg-[#25D366] px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-green-900/10 transition-transform hover:-translate-y-0.5"
        >
        ...
        <button
          onClick={() => setOpen((v) => !v)}
          className={`lg:hidden p-2 rounded-lg ${scrolled || open ? 'text-brand-blue' : 'text-white'}`}
